...
const CACHE = joinpath(@__DIR__, "cache", "argamasilla_2t_$(INIT_HOUR)z_6d.jls")   # one cache per hour
...
for lt in leads
    ...
    lt in REPORT_LEADS && @printf(
        "%-10s%-5d%-11.3f%-11.3f(%.2f, %.3f, %.3f, %.3f)\n",
        string(lt),
        ncases(t),
        crps_min([0, 1, 1, 0], t),
        crps_min(p, t),
...
points = DataFrame(
    init = DateTime[],
    lead = Hour[],             # at the 3-hourly steps
    observed = Float64[],
    raw = Float64[],
    mbm = Float64[],
)
for (d, ev) in zip(TEST_DATES, evals)
    ...
    for (fc, cfc) in zip(run.forecasts, cor.forecasts)
        ...
        push!(
            points,
            (run.timestamp, fc.lead_time, y, mean(fc.ensemble), mean(cfc.ensemble)),
        )
    end
end
...
@printf(
    "  3-hourly, raw ensemble mean        RMSE = %.2f   (N = %d)\n",
    rmse(points.raw, points.observed),
    nrow(points)
)
@printf(
    "  3-hourly, MBM-corrected mean       RMSE = %.2f\n",
    rmse(points.mbm, points.observed)
)
@printf(
    "  30-minute, ensemble CRPS raw       %.3f   (N = %d)\n",
    mean(allraw),
    length(allraw)
)
...
band!(
    ...
    hi .- 273.15;
    color = (:crimson, 0.18),
    label = "corrected ensemble, coldest to warmest member",
)
lines!(
    ax1,
    lead_h.(ev.times),
...
lines!(
    ax1,
    lead_h.(ev.times),
    [mean(view(ev.raw, i, :)) for i in eachindex(ev.times)] .- 273.15;
    color = :steelblue,
    linewidth = 1,
    linestyle = :dash,
...
for (d, e) in zip(TEST_DATES, evals), i in eachindex(e.times)
    push!(
        crpsdf,
        (
            Dates.value(e.times[i] - runs[d].timestamp) / 3.6e6,
            e.crps_raw[i],
            e.crps_corrected[i],
        ),
    )
...
bylead_crps =
    sort(combine(groupby(crpsdf, :lead), :raw => mean => :raw, :mbm => mean => :mbm), :lead)
lines!(
    ax2,
    bylead_crps.lead,
    bylead_crps.raw;
